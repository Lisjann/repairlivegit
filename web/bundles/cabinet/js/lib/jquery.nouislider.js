...
(function ($) {
    ...
    $.fn.noUiSlider = function (method, options) {
        ...
        helpers = {
            ...
            /* рисуем ограничитель */
            limitBarDraw: function (api) {
                if( api.limitBar ) {

                    var limit = api.options.limit;
                    var sliderW = api.slider.innerWidth();
                    var left = helpers.scale(limit[0], api.options.scale, sliderW);
                    var right = helpers.scale(limit[1], api.options.scale, sliderW);
                    var minWidth = 20;
                    if( Math.abs(right - left) < minWidth ) {
                        var middle = Math.abs(right - left) / 2;
                        left = left + middle - minWidth / 2;
                        right = left + middle + minWidth / 2;
                    }
        ...
        methods = {
            ...
            init:function () {
                ...
                return this.each(function () {
                    ...
                    api.limitBar = $('<div class="noUi-limitBar"></div>');

                    /* Append the middle bar */
                    s.connect ? api.connect.appendTo(api.slider) : api.connect = false;

                    // enable limit bar
                    if( s.limitBar && s.limit ) {
                        api.limitBar.appendTo(api.slider);
                    }
                    else {
                        api.limitBar = false;
                    }
                    ...
                    if (s.click) {
                        api.slider.click(functions.click)/*.find(':not(.noUi-midBar),:not(.noUi-limitBar)').click(functions.flse)*/;
                    }

                    helpers.connect(api);
                    helpers.limitBarDraw(api);
            ...
            limit:function(limit) {
                var api = dup($(this).data('api'));
                api.options.limit = limit;
                helpers.limitBarDraw(api);
            }
        ...
        functions = {
            ...
            move:function (e) {
                ...
                // проверка на лимит
                if( false && go && api.limitBar ) {
                    var sliderW = api.slider.innerWidth();
                    var value = helpers.deScale(a, api.options.scale, sliderW);
                    if( handle == api.low ) {
                        if( value < api.options.limit[0] ) {
                            go = false;
                        }
                    }
                    else if( handle == api.up ) {
                        if( value > api.options.limit[1] ) {
                            go = false;
                        }
                    }
                }
