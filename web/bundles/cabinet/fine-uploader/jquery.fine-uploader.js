...
//------------КОСТЫЛЬ ЧТОБЫ ПРИ КЛИКЕ НА КАРТИНКУ ОНА ВСТАВЛЯЛААСЬ В РЕДАКТИРУЕМОМ БЛОКЕ ПРИ ЗАГРУЗКЕ УЖЕ ЗАГРУЖЕННЫХ КАРТИНОК-----------------
					if( this._options.tinymceImgPaste ){
						$('li.qq-file-id-'+id+' img').live('click', function(){
							tinymce.execCommand('mceInsertContent',false,'<div itemtype="http://schema.org/ImageObject" itemscope="" class="img"><img src="' + sessionData.thumbnailUrl + '" alt="" width="660" itemprop="image" caption="false" /><p itemprop="description">Подпись к картинке</p></div><br/>');return false;
						});
					}
                }
                this._netUploaded++;
    ...
    qq.traditional = qq.traditional || {};
    qq.traditional.XhrUploadHandler = function(spec, proxy, o) {
        "use strict";
...
//------------КОСТЫЛЬ ВСТАВЛЯЕТ В РАДИОБАТОН URL ПРИ ЗАГРУЗКЕ НОВОЙ КАРТИНКИ-----------------
			$('li.qq-file-id-'+id+ ' .qq-edit-mainfile-selector').val( response.terget );

//------------КОСТЫЛЬ ЧТОБЫ ПРИ КЛИКЕ НА КАРТИНКУ ОНА ВСТАВЛЯЛААСЬ В РЕДАКТИРУЕМОМ БЛОКЕ ПРИ ЗАГРУЗКЕ НОВЫХ КАРТИНОК-----------------
			
				$('li.qq-file-id-'+id+ ' img').live('click', function(){
					tinymce.execCommand('mceInsertContent',false,'<div itemtype="http://schema.org/ImageObject" itemscope="" class="img"><img src="/' + response.terget + '" alt="" width="660" itemprop="image" caption="false" /><p itemprop="description">Подпись к картинке</p></div><br/>');return false;
				});
...
//------------СОЗДАЕМ НАЗВАНИЕ КЛАССА ДЛЯ РАДИОБАТОНА "ГЛАВНАЯ КАРТИНКА"-----------------
			mainFile: "qq-edit-mainfile-selector",
...
//------------СОЗДАЕМ ОБЪЕКТ ДЛЯ РАДИОБАТОНА "ГЛАВНАЯ КАРТИНКА"-----------------
mainFileEl = getTemplateEl(getFile(id), selectorClasses.mainFile),
				spec = {
...
mainFileEl = getTemplateEl(getFile(id), selectorClasses.mainFile),
            ...
            if (thumbnail) {
                if (thumbnailUrl) {
//------------ПРИСВАЕВАЕМ ОБЪЕКТУ РАДИОБАТОНА "ГЛАВНАЯ КАРТИНКА" СОДЕРЖАНИЕ URL-КАРТИНКИ-----------------
$(mainFileEl).val(thumbnailUrl);
//------------И ВЫДЕЛЯЕМ ЕГО ЕСЛИ СОДЕРЖИМОЕ РОВНО СОДЕРЖИМОМУ ИНПУТА ПОСТА ГЛАВНОЙ КАРТИНКЕ-----------------
if ( $('input#form_image').val() == thumbnailUrl )
	$(mainFileEl).prop({"checked":true});

//------------КОСТЫЛЬ ЧТОБЫ ПРИ КЛИКЕ НА КАРТИНКУ ОНА ВСТАВЛЯЛААСЬ В РЕДАКТИРУЕМОМ БЛОКЕ ПРИ ЗАГРУЗКЕ УЖЕ ЗАГРУЖЕННЫХ КАРТИНОК ТУТ ВСТАВЛЯЕТ 2 КАРТИНКИ СРАЗУ-----------------
/*$('li.qq-file-id-'+id+' img').live('click', function(){
	tinymce.execCommand('mceInsertContent',false,'<div itemtype="http://schema.org/ImageObject" itemscope="" class="img"><img src="' + thumbnailUrl + '" alt="" width="660" itemprop="image" caption="false" /><p itemprop="description">Подпись к картинке</p></div><br/>');return false;
});*/
